                             ...
                             const char *continue_msg)
{
    ...
    if (text_left < 2)
        text_left = 2; /* if window/screen is smaller than text */

    gdk_draw_rectangle(q->win, q->bg_gc, y, x + text_left, y + height / 2 - text_h / 2 - 4,
                       x + text_w + 7, text_h + 7);
    gdk_draw_rectangle(q->win, q->status_gc, y + 1, x + text_left + 1,
                       y + height / 2 - text_h / 2 - 3, x + text_w + 6, text_h + 6);
    for (i = 0; strs[i] && i < maxlines; i++)
    {
        pango_layout_set_text(layout, strs[i], -1);
        gdk_draw_layout(q->win, q->text_gc, x + text_left + 4,
                        y + height / 2 - text_h / 2 + i * (ascent + descent), layout);
    }

    /* Display Push Any Key... message */
    pango_layout_set_text(layout, continue_msg, -1);
    pango_layout_get_pixel_size(layout, &temp, NULL);
    gdk_draw_layout(q->win, q->text_gc, x + width / 2 - temp / 2,
                    y + height / 2 - text_h / 2 - descent + (i + 1) * (ascent + descent), layout);
...
void qiv_handle_event(GdkEvent *ev, gpointer data)
{
    ...
    if (!fullscreen)
    {
        ...
        //    g_print("position   : q->win_x = %d, q->win_y = %d, q->win_w = %d, mon = %d, typ =
    ...
    case GDK_CONFIGURE:
        if (magnify && !fullscreen)
        {
            gdk_window_get_root_origin(q->win, &magnify_img.frame_x, &magnify_img.frame_y);
    ...
    case GDK_BUTTON_RELEASE:
        ...
        case 1: /* left button pressed */
            ...
            if (q->drag)
            {
                ...
                if (q->drag > 1)
                {
                    qiv_drag_image(q, q->drag_win_x + move_x, q->drag_win_y + move_y, "(Drag)",
                                   NULL);
                    q->drag = 0;
                    break;
                }
    ...
    case GDK_KEY_PRESS:
        ...
        if (displaying_textwindow && !extcommand)
        { // [lc]
        ...
        { // [lc] todo: Backspace,
            ...
            if ((ev->key.keyval == GDK_KEY_Return) || (ev->key.keyval == GDK_KEY_KP_Enter) ||
                (jidx == 99))
            { /* 99 digits already typed */
                jcmd[jidx] = '\0';
                if (jumping)
                {
                    jump2image(jcmd);
            ...
            else if (ev->key.string && *(ev->key.string) != '\0')
            {
                if (jumping)
                {
                    ...
                    if ((jidx == 0 && (*(ev->key.string) != 'f' && *(ev->key.string) != 'F' &&
                                       *(ev->key.string) != 'b' && *(ev->key.string) != 'B' &&
                                       *(ev->key.string) != 't' && *(ev->key.string) != 'T')) ||
                        (jidx > 0 && (*(ev->key.string) < '0' || *(ev->key.string) > '9')))
                    {
                        jumping = 0;
        ...
        else
        {
            ...
            case 'e':
                ...
                snprintf(infotext, sizeof infotext,
                         center ? "(Centering: on)" : "(Centering: off)");
                if (center)
                    center_image(q);
                update_image(q, MOVED);
                break;
            ...
            case 'E':
            {
                ...
                if (lines)
                {
                    qiv_display_text_window(q, "(Exif Info)", (const char **)lines,
                                            "Press any key...");
            ...
            case 'm':
                scale_down = 0;
                maxpect ^= 1;
                snprintf(infotext, sizeof infotext, maxpect ? "(Maxpect: on)" : "(Maxpect: off)");
            ...
            case 'r':
                random_order ^= 1;
                snprintf(infotext, sizeof infotext,
                         random_order ? "(Random order: on)" : "(Random order: off)");
                update_image(q, REDRAW);
                break;
            ...
            case 'i':
                ...
                if (fullscreen)
                {
                    ...
                    snprintf(infotext, sizeof infotext,
                             statusbar_fullscreen ? "(Statusbar: on)" : "(Statusbar: off)");
                }
                else
                {
                    statusbar_window ^= 1;
                    snprintf(infotext, sizeof infotext,
                             statusbar_window ? "(Statusbar: on)" : "(Statusbar: off)");
                }
                update_image(q, MIN_REDRAW);
                break;
            ...
            case 'J':
                ...
                comment_window ^= 1;
                snprintf(infotext, sizeof infotext,
                         comment_window ? "(Comments: on)" : "(Comments: off)");
                update_image(q, MIN_REDRAW);
                break;
            ...
            case 's':
                ...
                snprintf(infotext, sizeof infotext, slide ? "(Slideshow: on)" : "(Slideshow: off)");
                update_image(q, MIN_REDRAW);
                break;
            ...
            case GDK_KEY_KP_Down:
                if (fullscreen)
                {
                    ...
                    qiv_drag_image(q, q->win_x, q->win_y - move_step, "(Moving up)",
            ...
            case GDK_KEY_KP_Up:
                if (fullscreen)
                {
                    ...
                    qiv_drag_image(q, q->win_x, q->win_y + move_step, "(Moving down)",
            ...
            case 't':
                ...
                scale_down ^= 1;
                snprintf(infotext, sizeof infotext,
                         scale_down ? "(Scale down: on)" : "(Scale down: off)");
                zoom_factor = maxpect ? 0 : fixed_zoom_factor; /* reset zoom */
                check_size(q, TRUE);
                update_image(q, REDRAW);
                break;
            ...
            zoom_out:
                if (magnify && !fullscreen)
                {
                    if (magnify_img.zoom > 2.0)
                    {
                        gint xcur, ycur;
                        magnify_img.zoom /= 1.1;
                        if (magnify_img.zoom < 2)
                            magnify_img.zoom = 2.0;
                        gdk_window_get_pointer(q->win, &xcur, &ycur, NULL);
            ...
            next_image:
                ...
                if (magnify && !fullscreen)
                {
                    ...
                    //                                         &magnify_img.frame_y);
                    //              printf(">>> frame %d %d\n", magnify_img.frame_x,
                    //              magnify_img.frame_y); setup_magnify(q, &magnify_img);
                }
                break;
            ...
            case 'w':
                watch_file ^= 1;
                snprintf(infotext, sizeof infotext,
                         watch_file ? "(File watching: on)" : "(File watching: off)");
                update_image(q, MIN_REDRAW);
                if (watch_file)
                {
                    // check every 100ms
            ...
            case GDK_KEY_F11:
                ...
                {
                    delay -= 1000;
                    snprintf(infotext, sizeof infotext, "(Slideshow-Delay: %d seconds (-1)",
                             delay / 1000);
                    update_image(q, MOVED);
                }
                else
                {
                    snprintf(infotext, sizeof infotext,
                             "(Slideshow-Delay: can not be less than 1 second!)");
                    update_image(q, MOVED);
                }
                break;
