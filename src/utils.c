...
void run_command(qiv_image *q, char *n, char *filename, int *numlines, const char ***output)
{
    ...
    *output = lines;
    ...
    if (before.st_size == after.st_size && before.st_ctime == after.st_ctime &&
...
#endif
        "    --rotate, -q x         Rotate 90(x=1),180(x=2),270(x=3) degrees clockwise (11 & 13 "
        "for conditional)\n"
            ...
            else
            {
                if (images >= max_image_cnt)
                {
                    ...
                    if (!image_names)
                        ...
                        image_names =
                            (char **)realloc(image_names, max_image_cnt * sizeof(char *));
                }
...
char *get_icc_profile(char *filename)
{
    ...
    /* is pic a tiff?*/
    else if ((pic_tst[0] == pic_tst[1]) && ((pic_tst[0] == 0x49) || (pic_tst[0] == 0x4d)) &&
             (pic_tst[2] == 0x2a) && (pic_tst[3] == 0x00))
    {
        uint16 count;
        unsigned char *data;
...
char **get_exif_values(char *filename)
{
    ...
    ExifTag gps_tags[] = {EXIF_TAG_GPS_LATITUDE,  EXIF_TAG_GPS_LATITUDE_REF,
