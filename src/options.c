...
static struct option long_options[] = {{"do_grab", 0, NULL, 'a'},
                                       {"brightness", 1, NULL, 'b'},
                                       {"contrast", 1, NULL, 'c'},
                                       {"delay", 1, NULL, 'd'},
                                       {"center", 0, NULL, 'e'},
    ...
    if (numeric_sort)
    {
        ...
        do
        {
            if (sufptr1 && (isdigit(*cp1) || isdigit(*cp2)))
            {
                ...
                if (diff && sufptr1 - cp1 <= namelen && sufptr2 - cp2 <= namelen)
                    return diff;
                namelen += ep1 - cp1;
                cp1 = ep1;
                cp2 = ep2 - 1;
...
void options_read(int argc, char **argv, qiv_image *q)
{
    ...
    while ((c = getopt_long(argc, argv, short_options, long_options, &long_index)) != -1)
    {
        ...
        case 'a':
            ...
            break;
        case 'b':
            q->mod.brightness = (checked_atoi(optarg) + 32) * 8;
            if ((q->mod.brightness < 0) || (q->mod.brightness > 512))
                usage(argv[0], 1);
            break;
        case 'c':
            q->mod.contrast = (checked_atoi(optarg) + 32) * 8;
            if ((q->mod.contrast < 0) || (q->mod.contrast > 512))
                usage(argv[0], 1);
            break;
        case 'd':
            delay = (int)(atof(optarg) * 1000);
            if (delay < 0)
            {
        ...
        case 'f':
            ...
            break;
        case 'g':
            q->mod.gamma = (checked_atoi(optarg) + 32) * 8;
...
#if GDK_PIXBUF_MINOR >= 12
        case 'l':
            autorotate = 0; /* turn autorotation OFF, default is ON */
            break;
#endif
        case 'm':
            maxpect = 1;
        ...
        case 'n':
            ...
            break;
        case 'o':
            image_bg_spec = optarg;
            break;
        case 'p':
            transparency = 1;
            break;
        case 'q':
            rotation = checked_atoi(optarg);
        ...
        case 'v':
            ...
            break;
        case 'w':
            q->win_w = fixed_window_size = checked_atoi(optarg);
            break;
        case 'x':
            to_root = 1;
            break;
        ...
        case 'z':
            ...
            break;
        case 'A':
            snprintf(select_dir, sizeof select_dir, "%s", optarg);
            break;
        case 'B':
            browse = 1;
            break;
        ...
        case 'T':
            ...
            break;
        case 'W':
            fixed_zoom_factor = (checked_atoi(optarg) - 100) / 10;
            zoom_factor = fixed_zoom_factor;
            break;
        case 'X':
            user_screen = checked_atoi(optarg);
    ...
    if (shuffle)
    {
        ...
        int i, p;
        for (i = 0; i < images; i++)
        { /* simple insertion sort, fine for small num */
